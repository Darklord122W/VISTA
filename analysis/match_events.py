#!/usr/bin/env python3
"""match_events.py — oracle-referenced event & detection recall for VISTA runs.

This is the root of the analysis dependency chain: every table in the paper
is downstream of the numbers this script produces.

Inputs
------
--oracle DIR   a `ref` run made with the ORACLE detector (yolo11x) at ring=0:
               every surviving frame processed. Its dets file defines ground
               truth. Frames are keyed by (camera_id, buf_pts) — deterministic
               across runs because replay skew injection is index/PTS
               deterministic.
--run DIR      one policy run dir (repeatable). Its dets file holds what the
               policy's detector saw on the frames the policy processed.

Ground truth
------------
* ORACLE DETECTIONS: every (frame, class, bbox) from the oracle with
  confidence >= --oracle-conf (default 0.40).
* ORACLE EVENTS: greedy IoU tracking over oracle detections per (camera,
  ...
  (IoU < --iou-track); an event must persist >= --min-frames oracle frames to
  count (kills single-frame flicker). Event time = first frame's buf_pts.

Metrics per policy run
----------------------
* coverage_vs_oracle : |oracle frames the policy also processed| / |oracle
  frames|. NOTE the denominator: frames on which the oracle emitted at least
  one conf>=0.40 detection (6415 for the office replay), NOT every frame the
  cameras produced (6882). See README.md, "Coverage denominator".
* det_recall_processed_frames : oracle detections on frames the policy
  processed that the policy matched (same class, IoU >= --iou-det).
* det_yield_overall : matched oracle detections / ALL oracle detections
  (coverage-weighted recall).
* event_recall@D : an event is recalled if the policy has a matching detection
  on that camera at an oracle FRAME TIME within [t0, t0+D].
* tta_recall@D : the same, charged the run's mean output age — the operator
  learns of the event only when the detection is EMITTED. Tables II and III
  use tta_recall; Table IV uses event_recall. They are not interchangeable.
* onset_delays_ms : first policy match time - event t0, for recalled events.
* per_cam_coverage.

Notes: PTS (buf_pts) is used ONLY as a frame identity/time key of the replay
timeline, not as a wall-clock quantity. IoU matching is in pixel coords (both
detectors run at 640x480 source resolution).
...
def load_jsonl(path):
    """Load a dets file. `path` may name dets.jsonl even when only
    dets.jsonl.gz exists — the .gz is preferred automatically. Kept as a
    positional-path API because clean_events/enriched_analysis call it."""
...
def load_run_dets(run_dir, stem="dets.jsonl"):
    """Preferred entry point: resolve .gz-or-plain inside a run directory."""
...
def run_mean_e2e(run_dir, warmup=5.0):
    ...
    (time-to-awareness): dets.jsonl has no emission stamps in the standard
    build, so each match is charged the run's mean output staleness.
    e2e_ms carries negative sentinels for batches whose capture stamp was
    unavailable; those rows are excluded, not clamped."""
